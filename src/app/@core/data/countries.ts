...
export class Countries {
  // LIST OF COUNTRIES NAME ONLY, NO REFERENCE
  private country_list_name_only = ['Afghanistan', 'Albania', 'Algeria', 'Andorra', 'Angola', 'Anguilla', 'Antigua &amp; Barbuda', 'Argentina', 'Armenia', 'Aruba', 'Australia', 'Austria', 'Azerbaijan', 'Bahamas'
    ...
    , 'Sierra Leone', 'Singapore', 'Slovakia', 'Slovenia', 'South Africa', 'South Korea', 'Spain', 'Sri Lanka', 'St Kitts &amp; Nevis', 'St Lucia', 'St Vincent', 'St. Lucia', 'Sudan', 'Suriname',
    'Swaziland', 'Sweden', 'Switzerland', 'Syria', 'Taiwan', 'Tajikistan', 'Tanzania', 'Thailand', 'Timor L\'Este', 'Togo', 'Tonga', 'Trinidad &amp; Tobago', 'Tunisia', 'Turkey', 'Turkmenistan', 'Turks &amp; Caicos', 'Uganda', 'Ukraine', 'United Arab Emirates', 'United Kingdom', 'Uruguay', 'Uzbekistan', 'Venezuela', 'Vietnam', 'Virgin Islands (US)', 'Yemen', 'Zambia', 'Zimbabwe'];

  // LIST OF COUNTRIES WITH STATES
  private countryStatesFull = [
    {
      code2: 'AF',
      code3: 'AFG',
      name: 'Afghanistan',
  ...
  /*
 * @desc GET LIST OF COUNTRIES BY NAME ONLY!
 */
  getCountriesByName(): Observable<string[]> {
    return observableOf(this.country_list_name_only);
  }

  // Get country with abbreviations
  getCountryAbbr(): Observable<any[]> {
    return observableOf(this.countryNameAbbr);
  }

  // return list of states for a particular country by name
  getCountryStates(countryName: string): Observable<any> {
    return observableOf(this.countryStatesFull.find(x => x.name === countryName));
  }
  // RETURNS STATES BY THE CODE 2 ABBREVIATION
  getCountryStatesByCodeTwo(code: string): Observable<any> {
    return observableOf(this.countryStatesFull.find(x => x.code2 === code));
  }
}

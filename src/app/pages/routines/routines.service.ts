...
export class RoutineService {
   ...
   * @param formData
   */
  fetch(routinesId): Observable<any> {
    let fullUrl = environment.apiUrl + 'admin/routines';
    if (routinesId !== 0) {
      fullUrl = environment.apiUrl + 'admin/routine/detail/' + routinesId;
    }
  ...
  add(fd) {
    ...
    return this.http.post<any>(environment.apiUrl + 'admin/routine/add', data)
      .pipe(
        catchError(_ => of(this.generalError)),
      );
  }
  ...
  update(fd) {
    const data = JSON.stringify(fd);
    return this.http.put<any>(environment.apiUrl + 'admin/routine/update', data)
      .pipe(
        catchError(_ => of(this.generalError)),
      );
  }
   ...
   */
  remove(id: number) {
    return this.http.delete<any>(environment.apiUrl + 'admin/routine/remove/' + id)
      .pipe(
        catchError(_ => of(this.generalError)),
      );
  }
